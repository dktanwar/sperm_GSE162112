...
res <- qc_SE(file = files, outdir = "./", name = names(files), ad1 = "TGGAATTCTCGGGTGCCAAGG")

json_files <- list.files(path = "./", pattern = "json", full.names = T)
names(json_files) <- gsub(pattern = "\\.json", replacement = "", x = basename(json_files))

trim_files <- list.files(path = "./", pattern = "_R1", full.names = T)
...
knitr::kable(make_general_df(json = json_files), col.names = NULL)
df1 <- make_general_df(json = json_files)
```
...
knitr::kable(make_before_filt_df(json = json_files), col.names = NULL)
df2 <- make_before_filt_df(json = json_files)
```
...
knitr::kable(make_after_filt_df(json = json_files), col.names = NULL)
df3 <- make_after_filt_df(json = json_files)
```
...
knitr::kable(make_filt_df(json = json_files), col.names = NULL)
df4 <- make_filt_df(json = json_files)
```
...
make_read_qual_plot(json = json_files, which = "before")
```

### After QC
```{r qc-report-12, warning=FALSE, message=FALSE, results='asis'}
make_read_qual_plot(json = json_files, which = "after")
```
...
make_base_ratio_line_plot(json = json_files, which = "before")
```

### After QC
```{r qc-report-15, warning=FALSE, message=FALSE, results='asis'}
make_base_ratio_line_plot(json = json_files, which = "after")
```
...
make_base_ratio_prop_plot(json = json_files, which = "before")
```

### After QC
```{r qc-report-18, warning=FALSE, message=FALSE, results='asis'}

make_base_ratio_prop_plot(json = json_files, which = "after")
```
...
res1 <- run_qc(fq = files)

res2 <- run_qc(fq = trim_files)
```
...
read_length_plot(before_trim = res1$read_length, after_trim = res2$read_length)
```
...
colnames(general)[1:2] <- c("Feature", "Value")

bqc <- df2
bqc$Sample <- sample
bqc$Category <- "Before QC"
colnames(bqc)[1:2] <- c("Feature", "Value")

aqc <- df3
aqc$Sample <- sample
aqc$Category <- "After QC"
colnames(aqc)[1:2] <- c("Feature", "Value")


fil <- df4
fil$Sample <- sample
fil$Category <- "Filtering"
colnames(fil)[1:2] <- c("Feature", "Value")

df <- rbind(general, bqc, aqc, fil)

library(gt)

gt(df[,-3], groupname_col = c("Category")) %>%
  tab_header(
    title = md(paste0("**", df$Sample[1], "**"))
  ) %>%
  tab_style(
    style = list(
  ...
  tab_style(
    style = list(
      cell_borders(sides = "right", color = "black", weight = px(2))
    ),
    locations = cells_body(
      # entire column
      columns = vars(Feature)
    )
  ) %>%
  cols_label(
    Feature = md("**Feature**"),       # recognizes markdown syntax
    Value = md("**Value**") # recognizes HTML syntax
  ) %>%
    opt_table_lines() %>%
  tab_options(column_labels.hidden = T, 
              ...
              heading.padding = px(0.7),
              row_group.padding = px(0.7), table.width = px(220))
```
